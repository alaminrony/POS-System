...
//        echo "<pre>";print_r($this->request->getVar());exit;
        ...
        if (!$this->validate($rules)) {
            ...
            if ($this->permission_model->role_create($postData)) {
                if ($roleid == '4') {
                    $userData = $this->permission_model->singleUser($user_id);
                    $customerExists = $this->permission_model->existsCustomer($user_id);
                    if ($customerExists == false) {
                        $cusInsert = $this->permission_model->insertUserToCustomer($userData);
                    } 
                }
