...
class AuthModel
{
    	...
    	public function checkUser($data = array())
	{
		$db      = db_connect();
        $builder = $db->table('user');
		$builder->select("user.id,CONCAT_WS(' ', user.firstname, user.lastname) AS fullname,user.email,user.image,user.firstname,user.lastname, user.last_login,user.last_logout,user.ip_address,user.status,user.is_admin,IF (user.is_admin=1, 'Admin', 'User') as user_level");
